...
from pandas import DataFrame, api, to_datetime, merge, read_csv
from numpy import ndarray, linalg, dot, argsort, vstack
...
class FeatureBuilder:
    ...
    @staticmethod
    def create_features_set(df: DataFrame, feature_cols: List[str]) -> Set[str]:
        """
        Creates a set of unique features in the format 'col:val' from the given DataFrame.
        """
        features_set = set()
        for col in feature_cols:
            for val in df[col].dropna().unique():
                features_set.add(f"{col}:{val}")
        return features_set
...
class LightFMRecommender:
    ...
    def _compute_country_product_stats(train_df: DataFrame) -> DataFrame:
        ...
        stats = merge(visits_mean, purchases_mean, on=['country', 'partnumber'], how='left').fillna(0)
    ...
    def train_lightfm_model(
        ...
        item_features_set = FeatureBuilder.create_features_set(products_df, item_feature_cols)
        user_features_set = FeatureBuilder.create_features_set(country_train_df, user_feature_cols)
        ...
        item_id_map, _ = dataset.mapping()[2:]
        index_to_product_id = {v: k for k, v in item_id_map.items()}
        product_ids = [index_to_product_id[i] for i in range(len(index_to_product_id))]

        user_id_map, _, _, _ = dataset.mapping()
        index_to_user_id = {v: k for k, v in user_id_map.items()}
        ...
        similar_products = self._precompute_similar_products_with_embeddings(
            product_embeddings, product_ids, country_train_df, 20, percentile_visits, percentile_purchases
        )
    ...
    def _precompute_similar_products_with_embeddings(
        ...
        *per-category* percentile thresholds for visits_mean and purchases_mean
        ...
        Steps:
        1) Compute stats with self._compute_country_product_stats(...).
        ...
        6) Build a dict {product_id: [top_sim]}.
        """
        ...
        if 'cod_section' not in country_stats.columns or 'family' not in country_stats.columns:
            ...
            country_stats = merge(country_stats, extra_info, on=['country', 'partnumber'], how='left')

        # 2) group by (cod_section, family) => find local percentile
        ...
        for (_, _, _), group_df in grouped:
            ...
            # 3) keep rows that exceed these local thresholds
            qualified = group_df[
        ...
        # 5) compute normalized embeddings
        candidate_norms = linalg.norm(candidate_embeddings, axis=1, keepdims=True)
        candidate_normalized = candidate_embeddings / candidate_norms

        all_norms = linalg.norm(product_embeddings, axis=1, keepdims=True)
        all_normalized = product_embeddings / all_norms

        # 6) build top_similar_map for each product
        top_similar_map = {}

        for i, product_id in enumerate(product_ids):
            base_vec = all_normalized[i:i + 1]
            similarities = dot(base_vec, candidate_normalized.T)[0]
            sim_indices = argsort(-similarities)[:top_n]
            top_candidates = [int(candidate_ids[idx]) for idx in sim_indices]
            top_similar_map[int(product_id)] = top_candidates

        return top_similar_map
    ...
    def run(
        ...
        similar_products_25 = self.train_lightfm_model(
            train_df, products_df, 25, user_feature, item_feature, no_components, percentile_visits, percentile_purchases
        )
        similar_products_29 = self.train_lightfm_model(
            train_df, products_df, 29, user_feature, item_feature, no_components, percentile_visits, percentile_purchases
        )
        similar_products_34 = self.train_lightfm_model(
            train_df, products_df, 34, user_feature, item_feature, no_components, percentile_visits, percentile_purchases
        )
        similar_products_57 = self.train_lightfm_model(
            train_df, products_df, 57, user_feature, item_feature, no_components, percentile_visits, percentile_purchases
        )
        ...
        return top_similar_products_by_country


class BoughtTogetherComputer:

    @staticmethod
    def compute_similarity_matrix(embeddings: ndarray) -> ndarray:
        """
        Returns a matrix of cosine similarities between all item embeddings.
        embeddings shape: (num_products, embedding_dim)
        """
        norms = linalg.norm(embeddings, axis=1, keepdims=True)
        normalized = embeddings / norms

        similarity_matrix = dot(normalized, normalized.T)
    ...
    def filter_low_usage_items(df: DataFrame, min_avg_visits_per_day: float = 1.0) -> DataFrame:
        ...
        if not api.types.is_datetime64_any_dtype(temp['timestamp_local']):
            temp['timestamp_local'] = to_datetime(temp['timestamp_local'], errors='coerce')
...
def train(no_components: int, percentile_visits: float, percentile_purchases: float, buy_threshold: float) -> None:
    ...
    if not os.path.exists(similar_products_item_based_path):
        ...
        embeddings = vstack(filtered_products_df['embedding'].to_numpy())
        product_ids = filtered_products_df['partnumber'].to_numpy()

        top_similar_item_based_map = LightFMRecommender()._precompute_similar_products_with_embeddings(
        ...
        with open(similar_products_item_based_path, 'w') as f:
            json.dump(top_similar_item_based_map, f, indent=4)

    if not os.path.exists(similar_products_lightfm_path):
        top_similar_products_by_country = LightFMRecommender().run(
            train_df, products_df, no_components, percentile_visits, percentile_purchases, [], []
        )

        with open(similar_products_lightfm_path, 'w') as f:
