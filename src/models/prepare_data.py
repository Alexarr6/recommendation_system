...
class Preprocessor:
    ...
    def preprocess_data(self, product_df: DataFrame, sessions_df: DataFrame, user_df: DataFrame) -> DataFrame:
        ...
        sessions_df['user_id'] = sessions_df['user_id'].apply(lambda x: str(int(x)) if pd.notna(x) else 'Unknown')
    ...
    def create_evaluation_subset(train_df: DataFrame, n_sessions: int = 5000, random_seed: int = 1) -> DataFrame:
        ...
        one_row_sessions = np.random.choice(chosen_sessions, size=int(len(chosen_sessions) * 0.5), replace=False)
        ...
        eval_df = pd.concat([one_row_sessions_df, several_row_sessions_df]).sort_index()
...
class ProductsInformationExtractor:
    ...
    @staticmethod
    def get_popular_products(train_df: pd.DataFrame, top_n: int = 100) -> List[str]:
        """
    ...
    @staticmethod
    def create_discount_map(products_df: pd.DataFrame) -> Dict[str, False]:
        """
    ...
    @staticmethod
    def build_user_bought_map(train_df: pd.DataFrame) -> Dict[str, List[int]]:
        """
    ...
    @staticmethod
    def build_user_not_bought_map(train_df: pd.DataFrame) -> Dict[str, Dict[int, dict]]:
        """
    ...
    def build_user_profiles(train_df: DataFrame) -> Dict[str, Dict[tuple, dict]]:
        ...
        for (user_id, fam, cod), products_list in temp_dict.items():
    ...
    def compute_out_of_stock_products(self, train_df: pd.DataFrame, stock_threshold: int, date_threshold: str) -> dict:
    ...
    def build_section_family_products(train_df: DataFrame) -> Dict[str, dict]:
        ...
        for (section, fam), group in views_df.groupby(['cod_section', 'family']):
            ...
            result_dict[str(section) + '_' + str(fam)] = {"top_viewed": viewed_list, "top_bought": []}
        ...
        for (section, fam), group in bought_df.groupby(['cod_section', 'family']):
            ...
            if str(section) + '_' + str(fam) not in result_dict:
                result_dict[str(section) + '_' + str(fam)] = {"top_viewed": [], "top_bought": bought_list}
            else:
                result_dict[str(section) + '_' + str(fam)]["top_bought"] = bought_list
    ...
    def compute_discount_ratio(session_df: DataFrame, products_df: DataFrame) -> Tuple[DataFrame, DataFrame]:
        ...
        session_df = session_df[['session_id', 'partnumber']].merge(products_df[['partnumber', 'discount']],
                                                                    on='partnumber', how='inner').drop_duplicates()
    ...
    def compute_sessions_types(
        ...
        new_season_session_df = pd.DataFrame(new_season_sessions_list, columns=['session_id'])
        new_season_session_df['session_type'] = '1'

        sales_sessions_session_df = pd.DataFrame(sales_sessions_list, columns=['session_id'])
        sales_sessions_session_df['session_type'] = '0'

        session_type_df = pd.concat([new_season_session_df, sales_sessions_session_df], ignore_index=True)
...
class ProductsMetricsComputer:
    ...
    @staticmethod
    def add_freshness_column(products_df: DataFrame, reference_date: pd.Timestamp = None) -> DataFrame:
        """
        ...
        if reference_date is None:
            reference_date = pd.to_datetime('2024-06-16')
    ...
    def main(self, train_df: DataFrame, products_df: DataFrame) -> DataFrame:
        ...
        product_metrics_df = pd.concat(product_metrics_list, ignore_index=True)
